...
class m_bigData extends CI_Model {
  ...
  public function loadData() {
    ...
    $sql = $this->db2->query("SELECT LZ_BD_CATEGORY_ID, CATEGORY_NAME, CATEGORY_ID, LISTING_TYPE, SELLER_ID, CONDITION, KEYWORD, INSERTED_DATE, ENTERED_BY, LISTING_FILTER, GLOBAL_ID FROM LZ_BD_CATEGORY");
   ...
   /* $totalData     = $query->num_rows();
    ...
    foreach ($query->result_array() as $row) { // preparing an array
      ...
      $nestedData   = array();
      $nestedData[] = "<div class='edit_btun' style='width: 135px; height: auto;'><a title='Delete Record' href='" . base_url() . "bigData/c_bigData/deleteBdCategory" . $row['LZ_BD_CATEGORY_ID'] . "' onclick='return confirm('Are you sure to delete?');' class='btn btn-danger btn-xs'><span class='glyphicon glyphicon-trash' aria-hidden='true'></span></a>&nbsp;<a title='Edit Record' href='".base_url()."bigData/c_bigData/editBdCategory/".$row['LZ_BD_CATEGORY_ID']."' class='btn btn-warning btn-xs'><span class='glyphicon glyphicon-pencil' aria-hidden='true'></span></a>&nbsp;<a title='Pull Sold Data' href='".base_url()."bigData/c_bigData/pullAllCategoryData/".$row['CATEGORY_ID']."' onclick='return confirm('Are you sure to want Pull Sold Data?');' class='btn btn-success btn-xs' target='_blank'><i class='fa fa-download' aria-hidden='true'></i></a>&nbsp;&nbsp;<a title='Pull Active Data' href='".base_url()."bigData/c_bigData/pullActiveData/".$row['CATEGORY_ID']."' onclick='return confirm('Are you sure to want Pull Active Data?');' class='btn btn-success btn-xs' target='_blank'><i class='fa fa-arrow-down' aria-hidden='true'></i></a>&nbsp;<a target='_blank' href='".base_url()."bigData/c_bigData/showDetailData/".$row['CATEGORY_ID']."'  class='btn btn-primary btn-xs' title='Show Detail'><i style=' cursor: pointer;' class='fa fa-external-link' aria-hidden='true'></i></a></div>";
  ...
  public function saveCategoryData() {
    ...
    $seller_id = trim(str_replace(array("'"), "''", $seller_id));    
    ...
    if($check_qry->num_rows()==0){
      ...
      $this->session->set_flashdata('warning', 'Already Inserted.');
    }
  ...
  public function pullActiveData() {
    ...
    $cat_qry = $this->db->query("SELECT * FROM LZ_BD_CATEGORY C  WHERE C.CATEGORY_ID = $cat_id");
    $cat_qry = $cat_qry->result_array();
    return $cat_qry;
  }
