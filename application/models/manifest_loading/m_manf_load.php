...
class m_manf_load extends CI_Model
    ...
    if ($ext !='xlsx'){
        ...
        $comma = ',';
        $qry_mt = "INSERT INTO LZ_MANIFEST_MT (LZ_MANIFEST_ID, LOADING_NO, LOADING_DATE, PURCH_REF_NO, SUPPLIER_ID, REMARKS, DOC_SEQ_ID, PURCHASE_DATE, POSTED, EXCEL_FILE_NAME, GRN_ID, PURCHASE_INVOICE_ID, SINGLE_ENTRY_ID, TOTAL_EXCEL_ROWS, MANIFEST_NAME, MANIFEST_STATUS, SOLD_PRICE, END_DATE, LZ_OFFER, MANIFEST_TYPE,EST_MT_ID)
         VALUES($LZ_MANIFEST_ID $comma $LOADING_NO $comma $loading_date $comma '$purch_ref' $comma '$supplier' $comma '$remarks' $comma $DOC_SEQ_ID $comma $purchase_date $comma '$POSTED' $comma '$file_name' $comma '$GRN_ID' $comma '$PURCHASE_INVOICE_ID' $comma '$SINGLE_ENTRY_ID' $comma $TOTAL_EXCEL_ROWS $comma '$manifest_name'  $comma '$MANIFEST_STATUS' $comma NULL $comma NULL $comma NULL $comma 0 $comma NULL)";
        ...
        foreach ($objWorksheet->getRowIterator() as $row)
        {
            ...
            $PO_DETAIL_RETIAL_PRICE = $objPHPExcel->getActiveSheet()->getCell('J'.$i)->getCalculatedValue();
            $SHIP_FEE = trim($objPHPExcel->getActiveSheet()->getCell('K'.$i)->getCalculatedValue());
            //$PO_DETAIL_RETIAL_PRICE = "''";
            $MAIN_CATAGORY_SEG1 = '';
            $SUB_CATAGORY_SEG2 = '';
            $object_name = $objPHPExcel->getActiveSheet()->getCell('L'.$i)->getCalculatedValue();
            $object_name = trim(str_replace("  ", ' ', $object_name));
            $object_name = trim(str_replace(array("'"), "''", $object_name));

            //var_dump($object_name, $BRAND_SEG3);exit;
                ...
                if(!empty($rs))
                    {
                        ...
                        $query = $this->db->query("SELECT SHIP_FEE from LZ_MANIFEST_DET WHERE UPPER(ITEM_MT_MFG_PART_NO) = '$ITEM_MT_MFG_PART_NO' ORDER BY LZ_MANIFEST_ID DESC");
                        $rs = $query->result_array();
...
//======================= Get Categories Starts ==========================
    ...
    // =================== Get Price Script Start ==========================
                ...
                if($query != NULL)
                {
                    ...
                    $exclude_word = '%20-lot%20-pcs';
                    $keywords=$query.$exclude_word;
                    ...
                    //$XML = new SimpleXMLElement(file_get_contents_curl($URL));
                    $ch = curl_init();
                    ...
                    if ($XML->ack != 'Failure' && $count > 0) {
                          ...
                          $ITEM_MT_DESC= $XML->searchResult->item->title;
                    }elseif($query == $UPC){
                        // $MPN = $row['MPN'];
                        // $MPN = str_replace("&", "and", $MPN);
                        // $MANUFACTURE = $row['MANUFACTURE'];
    ...
    // ============= Insertion of Categories and Price Start ================
                ...
                if(empty($UPC)){
                    $qry_det = "UPDATE LZ_MANIFEST_DET SET ITEM_MT_DESC = '$ITEM_MT_DESC',MAIN_CATAGORY_SEG1='$MAIN_CATAGORY_SEG1', SUB_CATAGORY_SEG2='$SUB_CATAGORY_SEG2', E_BAY_CATA_ID_SEG6='$E_BAY_CATA_ID_SEG6', PRICE=$A_PRICE,BRAND_SEG3 ='$BRAND_SEG3',S_PRICE=$S_PRICE WHERE LZ_MANIFEST_ID = $LZ_MANIFEST_ID AND ITEM_MT_UPC is null AND ITEM_MT_MFG_PART_NO = '$MPN'";
                }elseif(empty($MPN)){
            ...
            if($result){
                ...
                if(!empty($MPN) && !empty($E_BAY_CATA_ID_SEG6) && !empty($object_name)){
                    ...
                    $catalogue_check_query = $this->db->query("SELECT M.CATALOGUE_MT_ID, M.OBJECT_ID FROM LZ_CATALOGUE_MT M WHERE UPPER(M.MPN) = UPPER('$MPN') AND M.CATEGORY_ID = $E_BAY_CATA_ID_SEG6");
                    ...
                    if($catalogue_check_query->num_rows() > 0){
                        ...
                        $check_object_query = $this->db->query("SELECT O.OBJECT_ID FROM LZ_BD_OBJECTS_MT O WHERE UPPER(O.OBJECT_NAME) = UPPER('$object_name') AND O.CATEGORY_ID =  $E_BAY_CATA_ID_SEG6");
                        if($check_object_query->num_rows() > 0){
                            ...
                            $mt_qry = $this->db->query("INSERT INTO LZ_BD_OBJECTS_MT(OBJECT_ID, OBJECT_NAME, INSERT_DATE, INSERT_BY, CATEGORY_ID, ITEM_DESC) VALUES($object_id, '$object_name', $insert_date, $insert_by, $E_BAY_CATA_ID_SEG6, '$ITEM_MT_DESC')");  
                        }
                        ...
                        $mt_qry = $this->db->query("INSERT INTO LZ_CATALOGUE_MT(CATALOGUE_MT_ID, MPN, CATEGORY_ID, INSERTED_DATE, INSERTED_BY, CUSTOM_MPN, OBJECT_ID, MPN_DESCRIPTION, BRAND, PRICE, UPC) VALUES($cat_mt_id, '$MPN', $E_BAY_CATA_ID_SEG6, $insert_date, $insert_by, 0, $object_id,'$ITEM_MT_DESC', '$BRAND_SEG3', $A_PRICE, '$UPC')");
            ...
            /*=====  End of Add MPN in Catalogue_mt block  ======*/
        ...
        function manf_detail($lz_manifest_id)
    {
        ...
        $cost_sum = "SELECT ROUND(SUM(D.PO_DETAIL_RETIAL_PRICE * D.AVAILABLE_QTY),2) SUM_COST FROM LZ_MANIFEST_DET D WHERE D.LZ_MANIFEST_ID  = $lz_manifest_id"; 
        $cost_sum =$this->db->query($cost_sum);
        $cost_sum = $cost_sum->result_array();
        //  ========================== Manifest Data query END ===================================//
    ...
    function list_manf()
    {
        ...
        $qry_mt = "select t.* from LZ_MANIFEST_MT t where t.loading_date > TO_DATE('10/30/2016', 'mm/dd/yyyy') and t.single_entry_id is null ORDER BY t.lz_manifest_id DESC";
        }else{
        $qry_mt = "select t.* from LZ_MANIFEST_MT t where t.loading_date > TO_DATE('10/30/2016', 'mm/dd/yyyy') and t.single_entry_id is null and t.grn_id is not null ORDER BY t.lz_manifest_id DESC";
        }
...
//print_r($qry_mt);exit;
  ...
  public function manifest_sticker_print($item_code,$manifest_id,$barcode){
    $print_qry = $this->db->query("SELECT B.PO_DETAIL_LOT_REF, B.BARCODE_NO BAR_CODE ,SUBSTR(I.ITEM_DESC, 1, 80) ITEM_DESC, 'R#' || T.PURCH_REF_NO LOT_NO, B.UNIT_NO, (SELECT SUM(S.AVAILABLE_QTY) FROM LZ_MANIFEST_DET S WHERE S.LAPTOP_ITEM_CODE=I.ITEM_CODE AND S.LZ_MANIFEST_ID = T.LZ_MANIFEST_ID) LOT_QTY, 'SKU:' || I.ITEM_MT_BBY_SKU SKU,I.ITEM_MT_MFG_PART_NO MPN, I.ITEM_MT_UPC UPC FROM LZ_MANIFEST_MT T, LZ_BARCODE_MT B, ITEMS_MT I WHERE T.LZ_MANIFEST_ID = B.LZ_MANIFEST_ID AND B.ITEM_ID = I.ITEM_ID AND I.ITEM_CODE='$item_code' AND B.LZ_MANIFEST_ID = $manifest_id AND B.BARCODE_NO = $barcode");
    //$print_qry = $this->db->query("SELECT B.PO_DETAIL_LOT_REF, B.BARCODE_NO BAR_CODE , '~' || SUBSTR(I.ITEM_DESC, 1, 80) ITEM_DESC, 'R#' || T.PURCH_REF_NO LOT_NO, B.UNIT_NO, (SELECT SUM(S.AVAILABLE_QTY) FROM LZ_MANIFEST_DET S WHERE S.LAPTOP_ITEM_CODE=I.ITEM_CODE AND S.LZ_MANIFEST_ID = T.LZ_MANIFEST_ID AND ROWNUM = 1) LOT_QTY, 'SKU:' || I.ITEM_MT_BBY_SKU SKU FROM LZ_MANIFEST_MT T, LZ_BARCODE_MT B, ITEMS_MT I WHERE T.LZ_MANIFEST_ID = B.LZ_MANIFEST_ID AND B.ITEM_ID = I.ITEM_ID AND I.ITEM_CODE='$item_code' AND B.LZ_MANIFEST_ID = $manifest_id AND B.BARCODE_NO = $barcode");
  ...
  public function RePrintAllStickers(){
    ...
    $print_qry = $this->db->query("SELECT B.PO_DETAIL_LOT_REF, B.BARCODE_NO BAR_CODE ,SUBSTR(I.ITEM_DESC, 1, 80) ITEM_DESC, 'R#' || T.PURCH_REF_NO LOT_NO, B.UNIT_NO, (SELECT SUM(S.AVAILABLE_QTY) FROM LZ_MANIFEST_DET S WHERE S.LAPTOP_ITEM_CODE=I.ITEM_CODE AND S.LZ_MANIFEST_ID = T.LZ_MANIFEST_ID AND ROWNUM =1 GROUP BY S.ITEM_MT_DESC) LOT_QTY, 'SKU:' || I.ITEM_MT_BBY_SKU SKU, I.ITEM_MT_MFG_PART_NO MPN, I.ITEM_MT_UPC UPC FROM LZ_MANIFEST_MT T, LZ_BARCODE_MT B, ITEMS_MT I WHERE T.LZ_MANIFEST_ID = B.LZ_MANIFEST_ID AND B.ITEM_ID = I.ITEM_ID AND B.PRINT_STATUS = 1 AND B.LZ_MANIFEST_ID = $manifest_id ORDER BY B.BARCODE_NO ASC");
    // $query = $this->db->query("UPDATE LZ_BARCODE_MT SET PRINT_STATUS = 1 WHERE LZ_MANIFEST_ID = $manifest_id ");     
    //var_dump($print_qry); exit;
    return $print_qry->result_array();
  }
