...
public function get_history(){
 ...
 // $get_his_query = " SELECT * FROM (SELECT  D.LZ_DEKIT_US_DT_ID ,D.IDENT_DATE_TIME,C.BRAND,OB.OBJECT_NAME,D.CATALOG_MT_ID,D.CONDITION_ID,NVL(D.AVG_SELL_PRICE,0) SOLD_PRICE,C.CATEGORY_ID CATE, CO.COND_NAME,DECODE(D.MPN_DESCRIPTION, NULL, C.MPN_DESCRIPTION, D.MPN_DESCRIPTION) TITLE, C.MPN FROM LZ_BD_OBJECTS_MT OB,LZ_DEKIT_US_DT D, LZ_CATALOGUE_MT C ,LZ_ITEM_COND_MT CO WHERE D.CATALOG_MT_ID = C.CATALOGUE_MT_ID  AND D.LZ_MANIFEST_DET_ID IS NOT NULL AND D.OBJECT_ID = OB.OBJECT_ID AND D.CONDITION_ID = CO.ID AND D.CONDITION_ID = 3000 ";
...
public function get_mpns(){
  ...
  //if (!empty($catalog_mt_id) || $catalog_mt_id == null || $catalog_mt_id == 'null') {
    ...
    if ($identified_by != $user_id && $lock_status == 1) {
    ...
    }elseif(($identified_by == $user_id && $lock_status == 1) || ($identified_by =='' && $lock_status == 0)){
      ...
      if ($update_dekit) {
        ...
        ///////////////////////FOR SHOW PICTURE/////////////////////////////////////////
          $det = $this->db->query("SELECT D.BARCODE_PRV_NO, D.CONDITION_ID, D.DEKIT_REMARKS FROM LZ_DEKIT_US_MT M, LZ_DEKIT_US_DT D,LZ_BD_OBJECTS_MT O,BIN_MT B WHERE  D.BARCODE_PRV_NO = $child_barcode_no AND M.LZ_DEKIT_US_MT_ID = D.LZ_DEKIT_US_MT_ID AND D.OBJECT_ID = O.OBJECT_ID(+) AND D.BIN_ID = B.BIN_ID(+) ORDER BY D.BARCODE_PRV_NO ASC");
