...
if ($response->Ack !== 'Failure') {
   ...
   //echo $Title1."<br>";
    ...
    //var_dump ("sold ".$QuantitySold);
    
    $Quantity = trim(str_replace("  ", ' ', $item->Quantity));
    $remainingQty = $Quantity - $QuantitySold;
    ...
    if($remainingQty > 0 ){
        ...
        $barcode_query = $this->db->query("SELECT B.BARCODE_NO FROM LZ_BARCODE_MT B WHERE B.EBAY_ITEM_ID = $ItemID AND ITEM_ADJ_DET_ID_FOR_OUT IS NULL AND LZ_PART_ISSUE_MT_ID IS NULL AND LZ_POS_MT_ID IS NULL AND PULLING_ID IS NULL AND ROWNUM = 1 ")->result_array();
        ...
        if ($err) {
            ...
            foreach($product as $record):
                ...
                //var_dump($shopify_id);exit;
                $bar_tab_qry = $seed_qry = $this->db->query("SELECT B.LZ_MANIFEST_ID, B.ITEM_ID, B.CONDITION_ID FROM LZ_BARCODE_MT B WHERE B.EBAY_ITEM_ID =  $ItemID")->result_array();
                $lz_manifest_id = $bar_tab_qry[0]["LZ_MANIFEST_ID"];
                $item_id = $bar_tab_qry[0]["ITEM_ID"];
                $condition_id = $bar_tab_qry[0]["CONDITION_ID"];
