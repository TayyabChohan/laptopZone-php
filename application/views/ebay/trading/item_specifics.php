...
if ($response->Ack !== 'Failure') {
    ...
    foreach($namerecommendation as $NameRecommendation){
       ...
       //$con =  oci_connect('laptop_zone', 'S', 'wizmen-pc/ORCL') or die ('Error connecting to oracle');
        ...
        $query = $this->db->query("SELECT get_single_primary_key('CATEGORY_SPECIFIC_MT','MT_ID') MT_ID FROM DUAL")->result_array();
        ...
        $qry = $this->db->query("INSERT INTO CATEGORY_SPECIFIC_MT(MT_ID, EBAY_CATEGORY_ID, SPECIFIC_NAME, MARKETPLACE_ID, MAX_VALUE, MIN_VALUE, SELECTION_MODE, UPDATE_DATE, CUSTOM, CATALOGUE_ONLY) VALUES($mt_id $comma $ebay_category_id $comma '$specific_name' $comma $marketplace_id $comma $max_value $comma $min_value $comma '$selection_mode' $comma '$time_stamp' $comma 0 $comma 0)");
        //var_dump($qry);//exit;
        // $cmd =oci_parse($con, $qry);
        ...
        $value_recommendation= $NameRecommendation->ValueRecommendation;
            foreach($value_recommendation as $valuerecommendation){
                ...
                $query = $this->db->query("SELECT get_single_primary_key('CATEGORY_SPECIFIC_DET','DET_ID') DET_ID FROM DUAL")->result_array();
                // $result = oci_parse($con,$query);
                // oci_execute($result, OCI_DEFAULT);
                // $row = oci_fetch_array($result,OCI_ASSOC);
                $det_id = $query[0]['DET_ID'];
                ...
                $qry = $this->db->query("INSERT INTO CATEGORY_SPECIFIC_DET(DET_ID, MT_ID, SPECIFIC_VALUE) VALUES($det_id $comma $mt_id $comma '$specific_value')");
                //var_dump($qry);
                // $cmd =oci_parse($con, $qry);
                // oci_execute($cmd,OCI_DEFAULT);
                // oci_commit($con);
