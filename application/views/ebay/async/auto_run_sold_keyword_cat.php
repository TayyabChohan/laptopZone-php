...
//To Run php file using web browser you have to update php.ini file in Apache Folder
        ...
        $max_cat = "SELECT MAX(CATEGORY_ID) CATEGORY_ID FROM LZ_BD_RSS_FEED_URL WHERE KEYWORD IS NOT NULL AND CATLALOGUE_MT_ID IS NOT NULL";
        //echo $cat_qry; exit;
...
/*=====  End of if current cat id = max cat id restart loop condition  ======*/
        if($data != false && $restart_loop == false){
            ...
            $cat_query = "SELECT DISTINCT CATEGORY_ID FROM LZ_BD_RSS_FEED_URL U WHERE U.KEYWORD IS NOT NULL AND U.CATLALOGUE_MT_ID IS NOT NULL AND CATEGORY_ID ".$category_id." ORDER BY U.CATEGORY_ID ASC"; 
        }else{
            $cat_query = "SELECT DISTINCT CATEGORY_ID FROM LZ_BD_RSS_FEED_URL U WHERE U.KEYWORD IS NOT NULL AND U.CATLALOGUE_MT_ID IS NOT NULL ORDER BY U.CATEGORY_ID ASC"; 
        }
