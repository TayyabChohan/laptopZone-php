...
$service = new Services\FindingService([
    ...
    $catalogue_mt_id = $row['CATLALOGUE_MT_ID'];
...
if ($response->ack !== 'Failure') {
    ...
    // ============== Start Checking if record already exist =====================
                //$query = $this->db->query("SELECT * FROM $table_name D WHERE D.EBAY_ID = $itemId AND D.SALE_TIME = $endTime");
    // =========== End Checking if record already exist =====================
            // if($query->num_rows() == 0){
             if($data == false){

                // $select_query = "SELECT GET_SINGLE_PRIMARY_KEY('$table_name','LZ_BD_CATA_ID') LZ_BD_CATA_ID FROM DUAL";
                // $select =oci_parse($conn, $select_query);
                // oci_execute($select,OCI_DEFAULT);
                // $rs = oci_fetch_array($select, OCI_ASSOC);
                // $lz_bd_cata_id = $rs['LZ_BD_CATA_ID'];
//var_dump($rs['LZ_BD_CATA_ID']);exit;  
                // $query = $this->db->query("SELECT LAPTOP_ZONE.GET_SINGLE_PRIMARY_KEY('$table_name','LZ_BD_CATA_ID') LZ_BD_CATA_ID FROM DUAL");
    ...
    // =========== End Checking if record already exist =====================
            // if($query->num_rows() == 0){
             if($data == false){
                ...
                // $select_query = "SELECT MAX(LZ_BD_CATA_ID) LZ_BD_CATA_ID  FROM $table_name";
                // $select =oci_parse($conn, $select_query);
                // oci_execute($select,OCI_DEFAULT);
                // $rs = oci_fetch_array($select, OCI_ASSOC);

                //$query = $this->db->query("SELECT MAX(LZ_BD_CATA_ID) LZ_BD_CATA_ID  FROM $table_name ");
                //$rs = $query->result_array();
                // $lz_bd_cata_id = $rs['LZ_BD_CATA_ID'];
                // if(empty($lz_bd_cata_id)){
                //     $lz_bd_cata_id = 1;
