...
//To Run php file using web browser you have to update php.ini file in Apache Folder
        ...
        $max_cat = "SELECT MAX(CATEGORY_ID) CATEGORY_ID FROM LZ_BD_CATEGORY_TREE T WHERE T.PARENT_CAT_ID IS NOT NULL AND T.UPDATED IS NOT NULL AND T.CATEGORY_ID >= 152554 ORDER BY T.CATEGORY_ID ASC";
        //echo $cat_qry; exit;
...
/*=====  End of if current cat id = max cat id restart loop condition  ======*/
        if($data != false && $restart_loop == false){
            ...
            $cat_query = "SELECT T.CATEGORY_ID FROM LZ_BD_CATEGORY_TREE T WHERE T.PARENT_CAT_ID IS NOT NULL AND T.UPDATED IS NOT NULL AND T.CATEGORY_ID ".$category_id." ORDER BY T.CATEGORY_ID ASC"; 
        }else{
            $cat_query = "SELECT T.CATEGORY_ID FROM LZ_BD_CATEGORY_TREE T WHERE T.PARENT_CAT_ID IS NOT NULL AND T.UPDATED IS NOT NULL AND T.CATEGORY_ID >= 152554 ORDER BY T.CATEGORY_ID ASC"; 
        }
