...
if ($response->ack !== 'Failure') {
    ...
    // ===================== Start Checking if record already exist ==============================
        $select_query = "SELECT * FROM LZ_BD_CATAG_DATA_TEST D WHERE D.EBAY_ID = $itemId AND D.SALE_TIME = $endTime";
    ...
    // ============== Start Checking if record already exist =====================
                //$query = $this->db->query("SELECT * FROM LZ_BD_CATAG_DATA_TEST D WHERE D.EBAY_ID = $itemId AND D.SALE_TIME = $endTime");
    // =========== End Checking if record already exist =====================
            // if($query->num_rows() == 0){
            if($data == false){

                $select_query = "SELECT LAPTOP_ZONE.GET_SINGLE_PRIMARY_KEY('LZ_BD_CATAG_DATA_TEST','LZ_BD_CATA_ID') LZ_BD_CATA_ID FROM DUAL";
                $select =oci_parse($conn, $select_query);
                oci_execute($select,OCI_DEFAULT);
                $rs = oci_fetch_array($select, OCI_ASSOC);
                $lz_bd_cata_id = $rs['LZ_BD_CATA_ID'];
//var_dump($rs['LZ_BD_CATA_ID']);exit;  
                // $query = $this->db->query("SELECT LAPTOP_ZONE.GET_SINGLE_PRIMARY_KEY('LZ_BD_CATAG_DATA_TEST','LZ_BD_CATA_ID') LZ_BD_CATA_ID FROM DUAL");
...
foreach ($results as $pageNum => $response) {
    ...
    // =========== End Checking if record already exist =====================
            ...
            if($data == false){
                ...
                // $select_query = "SELECT MAX(LZ_BD_CATA_ID) LZ_BD_CATA_ID  FROM LZ_BD_CATAG_DATA_TEST";
                // $select =oci_parse($conn, $select_query);
                // oci_execute($select,OCI_DEFAULT);
                // $rs = oci_fetch_array($select, OCI_ASSOC);

                //$query = $this->db->query("SELECT MAX(LZ_BD_CATA_ID) LZ_BD_CATA_ID  FROM LZ_BD_CATAG_DATA_TEST ");
                //$rs = $query->result_array();
                // $lz_bd_cata_id = $rs['LZ_BD_CATA_ID'];
                // if(empty($lz_bd_cata_id)){
                //     $lz_bd_cata_id = 1;
