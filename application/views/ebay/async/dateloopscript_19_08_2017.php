...
/*=====  End of FOR RUNNING PHP FILE USING CLI (COMMAND LINE INTERFACE)  ======*/
    ...
    $select_query = "SELECT LAPTOP_ZONE.GET_SINGLE_PRIMARY_KEY('LZ_BD_CATEGORY_CALL_LOG','CALL_LOG_ID') CALL_LOG_ID FROM DUAL";
    $select = oci_parse($conn, $select_query);
    oci_execute($select,OCI_DEFAULT);
...
//foreach($data as $value){
    ...
    =============================================*/    
        if(!$flag){
            ...
            //$con =  oci_connect('laptop_zone', 'S', 'wizmen-pc/ORCL');
            if($i == 1){
                $time_query = "SELECT laptop_zone.DATE_TO_UNIX_TS(MAX(D.SALE_TIME)) AS UTIME FROM $main_table D WHERE D.MAIN_CATEGORY_ID =".$value['CATEGORY_ID'];
...
/* --Call log insertion start 19-08-2017-- */
    ...
    $min_max_query = "SELECT LAPTOP_ZONE.DATE_TO_UNIX_TS(MIN(D.INSERTED_DATE)) MIN_TIME, LAPTOP_ZONE.DATE_TO_UNIX_TS(MAX(D.INSERTED_DATE)) MAX_TIME FROM $table_name D WHERE D.MAIN_CATEGORY_ID = ".$value['CATEGORY_ID'];
    ...
    $insert_qry = "INSERT INTO LZ_BD_CATEGORY_CALL_LOG (CALL_LOG_ID, CATEGORY_ID, LISTING_FILTER, MIN_TIME, MAX_TIME, EXECUTION_TIME, RECORDS_PER_MIN, TOTAL_RECORDS, PER_DAY_AVG, INSERTED_DATE, CALL_STATUS) VALUES($call_log_id $comma $cat_id $comma '$listing_filter' $comma $min_time $comma $max_time $comma '$execution_time' $comma $per_mint_records $comma $total_records $comma $avg_records_data $comma $inserted_date $comma 1)"; 
    ...
    $verify_pro = "CALL PRO_RECOGNIZE_SOLD_DATA($cat_id)";
