...
foreach($data as $value){
    ...
    =============================================*/    
        if(!$flag){
            $time_query = $this->db->query("SELECT DATE_TO_UNIX_TS(MAX(D.SALE_TIME)) AS UTIME FROM LZ_BD_CATAG_DATA D WHERE D.MAIN_CATEGORY_ID =".$value['CATEGORY_ID'] and upper(D.STATUS)="ACTIVE");
...
// EndTimeTo =  Limits the results to items ending on or before the specified time. Specify a time in the future.           
        ...
        include 'activeitemswithdatefilter.php';
