...
foreach ($res as $value) {
    ...
    $request->q = $value['EBAYID'];

    //$request->q = 'iphone';
    // $request->sort = '-price';
    $request->limit = '1';
    //$request->category_ids = '179697';
    $request->offset = '0';
   ...
   // printf("\nStatus Code: %s\n\n<br>", $response->getStatusCode());
    if (isset($response->errors)) {
        foreach ($response->errors as $error) {
            if($error->errorId === 2001){//too many requet error
                ...
                $this->load->view('ebay/browse/01-search-for-items');//call again
            }else{
                // $this->db2->query("UPDATE LZ_BD_MOST_WATCHED_ITEMS SET GETEPID = 1 WHERE WATCH_ID = $watch_id");
                printf(
                "%s: %s\n%s\n\n",
    ...
    if ($response->getStatusCode() === 200) {
        foreach ($response->itemSummaries as $item) {
            ...
            if(!empty(@$item->epid)){
                $epid = $item->epid;
                $this->db2->query("UPDATE LZ_BD_MOST_WATCHED_ITEMS SET EPID = $epid , GETEPID = 1 WHERE WATCH_ID = $watch_id");
            }else{
               $this->db2->query("UPDATE LZ_BD_MOST_WATCHED_ITEMS SET GETEPID = 1 WHERE WATCH_ID = $watch_id"); 
            }
        }
