...
foreach ($data as $val){
    ...
    // $itemFilter->value[] = 'FixedPriceItem';
    ...
    */
    ...
    $request->sortOrder = 'EndTimeSoonest';//https://developer.ebay.com/devzone/finding/callref/types/SortOrderType.html
    ...
    if ($response->ack !== 'Failure') {
       ...
       // ============== Start Checking if record already exist =====================
            $query = $this->db->query("SELECT * FROM LZ_WEEKLY_SOLD_DATA D WHERE D.EBAY_ID = $itemId AND D.SALE_TIME = $endTime");
    // =========== End Checking if record already exist =====================
            if($query->num_rows() == 0){
                //$this->db->query("TRUNCATE TABLE WIZ_ALL_TABLES_PK");
                $query = $this->db->query("SELECT GET_SINGLE_PRIMARY_KEY('LZ_WEEKLY_SOLD_DATA','LZ_SOLD_ID') LZ_SOLD_ID FROM DUAL");
                $rs = $query->result_array();
                $lz_sold_id = $rs[0]['LZ_SOLD_ID'];
                $insert_qry = "INSERT INTO LZ_WEEKLY_SOLD_DATA (LZ_SOLD_ID, CATEGORY_ID, EBAY_ID, TITLE, CONDITION_ID, CONDITION_NAME, ITEM_URL, SALE_PRICE, LISTING_TYPE, START_TIME, SALE_TIME, INSERTED_DATE, SELLER_ID, FEEDBACK_SCORE, CATEGORY_NAME, CURRENCY_ID,GLOBAL_ID,MAIN_CATEGORY_ID,ITEM_LOCATION,POSTAL_CODE,BID_COUNT,ITEM_COUNTRY,SHIPPING_COST,SHIPPING_TYPE,ITEM_ID,STATUS,KEYWORD) VALUES($lz_sold_id $comma $categoryId $comma $itemId $comma '$title' $comma '$condition_Id' $comma '$condition_name' $comma '$viewitemurl' $comma $price $comma '$listingType' $comma $startTime $comma $endTime $comma $inserted_date $comma '$sellerUserName'  $comma '$feedbackscore' $comma '$categoryName' $comma '$currencyId' $comma '$globalId'  $comma '$main_category_id' $comma '$item_location' $comma '$postal_code' $comma '$bid_count' $comma '$item_country' $comma $shippingServiceCost $comma '$shippingType' $comma '$item_id' $comma '0' $comma '$key')";
            }else{
                $insert_qry = "UPDATE LZ_WEEKLY_SOLD_DATA SET TITLE = '$title', CONDITION_ID = '$condition_Id', CONDITION_NAME = '$condition_name', SALE_PRICE = $price, LISTING_TYPE = '$listingType', START_TIME = $startTime, SALE_TIME = $endTime, INSERTED_DATE = $inserted_date, FEEDBACK_SCORE = '$feedbackscore' ,MAIN_CATEGORY_ID = '$main_category_id', BID_COUNT = '$bid_count',ITEM_ID = '$item_id', KEYWORD = '$key' WHERE EBAY_ID = $itemId AND SALE_TIME = $endTime";
            }
            ...
            $query = $this->db->query("SELECT GET_SINGLE_PRIMARY_KEY('LZ_WEEKLY_SOLD_DATA','LZ_SOLD_ID') LZ_SOLD_ID FROM DUAL");
                $rs = $query->result_array();
                $lz_sold_id = $rs[0]['LZ_SOLD_ID'];
                $insert_qry = "INSERT INTO LZ_WEEKLY_SOLD_DATA (LZ_SOLD_ID,ITEM_ID,STATUS) VALUES($lz_sold_id $comma '$item_id' $comma '1')"; 
