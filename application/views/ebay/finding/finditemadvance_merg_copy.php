...
if(empty($revise_ebay_id)){
 // if ebay id is not null than its a revise item call
    if(!empty($upc)){
    ...
    if(!empty($account_type)){
     $itemFilter->value[] = $config['production']['seller_name'];   
    }else{
...
if ($response->ack !== 'Failure') {
    ...
    if($response->searchResult->count > 0){
        ...
        $condition_id[]='';
        $condition_Name[]='';
        $account_name=[];// added by adil error:Message: [] operator not supported for strings
        $account_name[]='';
       ...
       // $this->session->set_userdata('ebay_id', $ebay_id);
        ...
        $result['user_id'] = $uer_id;
        $result['userName'] = $userName;
// var_dump($ebay_id);
// exit;
//         echo json_encode($result['ebay_id'] );

//         return json_encode($ebay_id);
// //               var_dump($bestOfferCheckbox);
// // exit;
         $this->load->view('tolist_view/v_listing_confirm_merg_copy',$result);
             exit;
