        ...
        <div class="col-sm-12">
          <div class="box">
            ...
            <div class="box-body">
              ...
              <form action="<?php echo base_url(); ?>rssFeed/c_rssFeed/CatFiltercatAuction" method="post" accept-charset="utf-8">
                ...
                <label for="feed_url">Category:</label>
                    <div class="form-group">
                      ...
                      if(@$row['CATEGORY_ID'] == @$data['selected_cat']){
                      ...
                      ?>
                        <option value="<?php echo @$row['CATEGORY_ID'];?>" <?php echo $selected;?>><?php echo @$row['CATEGORY_ID'].'-'.@$row['CATEGORY_NAME'].' ('.@$row['TOTAL_QTY'].')';?></option>
                      <?php endforeach;?>
                      </select>
                    </div>                                     
                </div> 
...
if(dataTable != ''){
     ...
     dataTable = $('#rssFeedTable').DataTable({
      ...
      "bAutoWidth": false,
      "ajax":{
        url: '<?php echo base_url();?>rssfeed/c_rssfeed/loadCatAuctionFilter', 
        type: 'post',
        dataType: 'json',
        data:{'cat_id':cat_id}
      },
      "columnDefs":[{
        ...
        "rowCallback": function( row, data, dataIndex){
            if(data){
                ...
                $(row).find(".viewed").closest( "tr" ).addClass('viewedtr');
                }
              }
...
=======================================================*/
  ...
  // $(".loader").show();
  $.ajax({
      url: '<?php echo base_url(); ?>rssfeed/c_rssfeed/updateCatAuction',
      type: 'post',
      dataType: 'json',
      ...
      success: function(data){
          ...
          console.log('RSS Feed Updated');
            ...
            for(var i = 0; i<data.length; i++){
          	 ...
          	 var url4 = '&_nkw='+data[i].EBAY_KEYWORD;
          	 var url5 = '&LH_Complete=1&LH_Sold=1&LH_PrefLoc=1';
          	 var sold_history_url = htmlEntities(url1+url2+url3+url4+url5);
             var url5 = '&LH_PrefLoc=1';
             var active_item_url = htmlEntities(url1+url2+url3+url4+url5);
          ...
          var similarFeed_url = '<?php echo base_url()."rssfeed/c_rssfeed/similarFeed/"; ?>'+data[i].CATALOGUE_MT_ID;
          /*=====  End of custom BIN URL block  ======*/

              	var td = '<td> <div style="display: inline-block; position: relative; width: 180px; padding: 4px;"> <div class="trash_button" style="float: left;padding-right: 5px;" id="'+data[i].FEED_ID+'_20"> <button title="Discard" class="btn btn-danger btn-xs flag-trash" style="width: 25px;" id="'+data[i].FEED_ID+'" fid="20" ct_id="'+data[i].CATEGORY_ID+'"><i class="fa fa-trash-o text text-center" aria-hidden="true"> </i> </button> </div> <div class="usd_button" style="float: left; padding-right: 5px;" id="'+data[i].FEED_ID+'_23"> <button title="Select for Purchase" class="btn btn-warning btn-xs flag-usd" style="width: 25px; " id="'+data[i].FEED_ID+'" fid="23" ct_id="'+data[i].CATEGORY_ID+'"><i class="fa fa-usd text text-center" aria-hidden="true"></i> </button> </div> <div class="info_link btn btn-success btn-xs" style="float: left; margin-right: 5px;width: 25px;color:white!important;" id="" title = "Sold History"> <a style="color:white!important; " href="'+sold_history_url+'" target= "_blank" ><i class="fa fa-info" aria-hidden="true"></i></a> </div><div class="bin_link btn btn-primary btn-xs" style="float: left; margin-right: 5px;width: 25px;color:white!important;" id="" title = "BIN"> <a style="color:white!important; " class="eBayCheckout" id="<?php echo @$row['EBAY_ID'];?>" href="'+bin_url+'" target= "_blank" ><i class="fa fa-paypal" aria-hidden="true"></i></a></div><div class="bin_link btn btn-info btn-xs" style="float: left; margin-right: 5px;width: 25px;color:white!important;" id="" title = "Show Similar Item"> <a style="color:white!important; " class="" id="<?php echo @$row['EBAY_ID'];?>" href="'+similarFeed_url+'" target= "_blank" ><i class="fa fa-paypal" aria-hidden="true"></i></a><button title="Items to Estimate" class="btn btn-primary btn-xs find_item m-l-5 m-r-5" style="" id="<?php echo @$row['EBAY_ID'];?>"><i class="fa fa-download" aria-hidden="true"></i></button></div></div> </td> '; 
              	td += '<td><div class = "item_desc" id="link_other" style="width: 300px;">'+data[i].ITEM_DESC+'</div></td> ';
                 ...
                 td += '<td><a href="'+sold_history_url+'" target= "_blank" >'+parseFloat(data[i].SOLD_AVG).toFixed(2)+'</a></td>';
                 td += '<td><a href="'+active_item_url+'" target= "_blank" >'+parseFloat(data[i].ACTIVE_AVG).toFixed(2)+'</a></td>';
            ...
            =============================================*/
              // var win = window.open(bin_url, '_blank');
              // if (win) {
...
=============================================*/
  $(document).on('click','.ebay_link',function(){
    ...
    var feedId = this.id;
        $(".loader").show();
          $.ajax({
              url: '<?php echo base_url(); ?>rssfeed/c_rssfeed/markedAsView', 
              type: 'post',
              dataType: 'json',
              data : {
                      'feedId':feedId
              ...
              success:function(data){
                if(data){
                  $(".loader").hide();
                  $('#'+feedId).closest( "tr" ).addClass('viewedtr');
                }else{
                  //console.log(data[0].FEED_NAME);
                //$('#UpdateKeyword').modal('show');
                $(".loader").hide();
