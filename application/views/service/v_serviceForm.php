...
<form action="<?php echo base_url(); ?>service/c_serviceForm/addService" method="post"  onsubmit="setTimeout(function(){ location.reload(); }, 1000);" accept-charset="utf-8" target="_blank">
                ...
                <div class="col-sm-12">
                  ...
                  <div class="form-group">
                   <label for="Doc No" class="control-label">DOC No:</label>
                     <input type="number" class="form-control" id="doc_no" name="doc_no" value="<?php echo $doc_no[0]['DOC_NO'];?>" readonly>
                  </div>
                  </div>
                ...
                <div class="col-sm-2 ">
                  <div class="form-group">
                   ...
                   <div class="input-group">
                    ...
                    <input type="text" class="form-control" id="return_date" name="return_date" value="<?php echo date('m/d/Y');?>" data-date-format="mm/dd/yyyy" >
                  </div>
                  </div>
                </div>
                  ...
                  <div class="col-sm-12 form-group">
                    <div class="col-sm-3">
                      <div class="form-group ">
                        ...
                        <select name="equip_type"  id="equip_type" class="form-control"> <?php  foreach(@$equipment['equipQry'] as $equip):?> <option value="<?php echo $equip['EQUIP_ID'];?>"><?php echo $equip['EQUIP_TYPE_DESC'];?></option> <?php endforeach; ?> </select>
                      </div>
                    ...
                    <div class="col-sm-4">
                      <div class="form-group">
                        ...
                        <input type="text" class="form-control" id="equip_desc" name="equip_desc" required>
                      </div>
                     ...
                     <div class="col-sm-4">
                      <div class="form-group">
                        ...
                        <input type="" class="form-control" id="imei" name="equip_imei"  >
                      </div>
                  ...
                  <div class="col-sm-12">
                    <div class="col-sm-7">
                        <div class="form-group">
                          ...
                          <textarea name="job_det" class="form-control" required></textarea>
                        </div>
  ...
  <script text="javascript">
   ...
   /*====================================================
    ...
    //     var num = this.id.match(/\d+/);
    //     var serv_net_price = $('#serv_net_price'+num+'').val();
    //     var sum = 0;
    //     sum = parseFloat(sum);
    //       $("input[class *= 'net']").each(function(){
    //       sum += parseFloat($(this).val());
    ...
    $('#serveCharges').on('blur',function(){
      ...
      if(isNaN(servCharge)){
        $('#servePlusNet').val(total_Net.toFixed(2));
      }
      else{
    ...
    $('#option1').on('click',function(){
       ...
       $('#opt2').val('');
       $('#pos_card_number').prop('required',false);
       var tax_amt = parseFloat($('#taxAmt').val());

       $('#s_tax_amt').val(tax_amt.toFixed(2));
       
       var grand_amt = parseFloat($('#grand').val());

       $('#serv_total_amount').val(parseFloat(grand_amt.toFixed(2)));
     ...
     $('#option2').on('click',function(){
       ...
       var grand_amt = parseFloat($('#grand').val());
       $('#serv_total_amount').val(grand_amt.toFixed(2));
       
    });

    if($('#serveCharges') == ''){
        parseFloat($('#serveCharges').val(0));
      }
    $('#exempt').on('click',function(){
      var checkbox = $('#exempt').prop('checked');
      if(checkbox == true){
            var tax_amt =  parseFloat($('#taxAmt').val());
            var grand = parseFloat($('#grand').val());
            var exempted = parseFloat(grand - tax_amt);
    ...
    $('#td_amount').on('blur',function(){
      ...
      var checkbox = $('#advance').prop('checked');
      if(checkbox==true){
        var refunded = parseFloat((tender - adv).toFixed(2));
      ...
      if(checkbox==false){
        if(parseFloat(tender) < parseFloat(net)){
          ...
          var refunded = parseFloat((tender - net).toFixed(2));
          $('#serv_refund').val(refunded);
        }else{
          var refunded = parseFloat((tender - net).toFixed(2));
          $('#serv_refund').val(refunded);
