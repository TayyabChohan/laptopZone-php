...
$(".show_detail").click(function(){
  ...
  // return false;
      ...
      $.ajax({
        dataType: 'json',
        type: 'POST',
        url:'<?php echo base_url(); ?>checklist/c_checklist/test_detail',
        data: { 'test_id' : test_id},
       success: function (data) {
         if(data != ''){ 
          //alert(data[0]);return false;

// if(data[0].SELECTION_MODE =='List'){ 
//   var i;
//   for (i = 0; i < data.length; i++) { alert(data[i].CHECK_VALUE)}   return false;       
// }
        //var i;
        ...
        if(data[0].SELECTION_MODE=='List'){
          var i;
          var label = '<label style="margin-left:40px;font-size:16px;margin-bottom:15px;">List Options:</label><br>';
          myWindow.document.write(label);
          for(i = 0;i<data.length;i++){myWindow.document.write('<li style="margin-left:72px;"><span class="text">'+data[i].CHECK_VALUE+'</span></li><br>')}
          ...
          myWindow.document.write('<div class="row"><div class="col-sm-12"><div class="box"><div class="box-header with-border" style="background-color: #3c8dbc !important;color:#fff !important;"><h4 class="box-title">Check List Details</h4></div><div class="box-body"><div class="col-xs-6"><div class="form-group"><label for="">Testing Check Names:</label><span class="text"> &nbsp;&nbsp;'+data[0].CHECK_NAME+'</span></div></div><div class="col-xs-6"><div class="form-group"><label for="">Selection Mode:</label><span class="text"> &nbsp;&nbsp;'+data[0].SELECTION_MODE+'</span></div></div></div></div></div>');
          checkDetail();
...
/*=============================================
=            show checklist detail            =
=============================================*/

$(".checklist_detail").click(function(){
  var cat_id = this.id;
  var manifest_list = $("#manifest_list").val();
   // alert(manifest_list); 
   // return false;
      ...
      $.ajax({
        dataType: 'json',
        type: 'POST',
        url:'<?php echo base_url(); ?>tester/c_tester_screen/showChecklistDetail',
        data: { 'cat_id' : cat_id,
       ...
       success: function (data) {
         if(data != ''){ 
          //alert(data[0]['CATEGORY_ID']);return false;
...
// }
       ...
       myWindow.document.write('<html><head><title>Laptopzone | Check List Details</title><link rel="stylesheet" type="text/css" href="<?php echo base_url('assets/bootstrap/css/bootstrap.min.css');?>"><link rel="stylesheet" href="<?php echo base_url('assets/dist/css/AdminLTE.min.css');?>"><link rel="stylesheet" href="<?php echo base_url('assets/dist/css/skins/_all-skins.min.css');?>"></head><body>');
          ...
          var i;
          var label = '<div class="box"><div class="box-header w-border" style="background-color: #3c8dbc !important;color:#fff !important;"><h4 class="box-title">Check List Details</h4></div></div><div class="col-xs-6"><div class="form-group"><label for="">Category ID:</label><span class="text"> &nbsp;&nbsp;'+data[0]['CATEGORY_ID']+'</span></div></div></div><div class="col-xs-6"><div class="form-group"><label for="">Category Name:</label><span class="text"> &nbsp;&nbsp;'+data[0]['BRAND_SEG3']+'</span></div></div><br><div class="row"><div class="col-sm-12" ><div class="form-group" style="border: 1px solid #ccc !important; padding: 3px 10 !important;"><label for="">Checklist Name:</label>'; myWindow.document.write(label);


          for(i = 0;i<data.length;i++){
          
          myWindow.document.write('<li class="text"> &nbsp;&nbsp;'+data[i]['CHECKLIST_NAME']+'</li>');           
...
============================================*/
  ...
  $(".rot_my_pic").click(function(){

 var img = $(this).parents('.imgCls').find('img');
 var rota_status = $(this).parents('.imgCls').find('input').attr('cla');
  ...
  $(".save_img").click(function(){
    ...
    $.ajax({
      ...
      data: { 'get_pic' : get_pic,'get_url' : get_url ,'stat_rot' : stat_rot,'child_barcode' : child_barcode 
    },
     success: function (data) {
      $(".loader").hide();
