...
do{
    ...
    $requestXmlBody .= "<OrderIDArray>$order_ids</OrderIDArray>";
    ...
    if ($errors->length > 0) {
        ...
        'shortMsg'     => @$shortMsg->item(0)->nodeValue,
        'longMsg' => @$longMsg->item(0)->nodeValue
        );
        $this->session->set_userdata($newdata);
        $this->session->set_userdata('response', 'error');
        ...
        $this->session->set_userdata('response', 'success');
        if(isset($_GET['debug']))
        {  
