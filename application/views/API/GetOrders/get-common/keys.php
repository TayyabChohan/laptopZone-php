<?php
/*  © 2013 eBay Inc., All Rights Reserved */ 
/* Licensed under CDDL 1.0 -  http://opensource.org/licenses/cddl1.php */
    //show all errors
    error_reporting(E_ALL);

    // these keys can be obtained by registering at http://developer.ebay.com
    
    $production         = true;   // toggle to true if going against production
    //$compatabilityLevel = 717;    // eBay API version
    $compatabilityLevel = 979;
    if ($production) {
        $devID = '702653d3-dd1f-4641-97c2-dc818984bfaa';   // these prod keys are different from sandbox keys
        $appID = 'SajidKha-eRashanp-PRD-42f839365-d3726265';
        $certID = 'PRD-2f839365e792-a58f-4449-b744-ecb4';
        //set the Server to use (Sandbox or Production)
        $serverUrl = 'https://api.ebay.com/ws/api.dll';      // server URL different for prod and sandbox
        //the token representing the eBay user to assign the call with
        $userToken = 'AgAAAA**AQAAAA**aAAAAA**7zvAVw**nY+sHZ2PrBmdj6wVnY+sEZ2PrA2dj6AElIGpD5aGqQidj6x9nY+seQ**u0gDAA**AAMAAA**';          
    } else {  
        // sandbox (test) environment
        $devID = '702653d3-dd1f-4641-97c2-dc818984bfaa';   // these SB keys are different from prod keys
        $appID = 'SajidKha-eRashanp-SBX-32f839347-808d8869';
        $certID = 'SBX-2f8393475fdc-e2ab-44b1-867f-e199';
        //set the Server to use (Sandbox or Production)
        $serverUrl = 'https://api.sandbox.ebay.com/ws/api.dll';
        // the token representing the eBay user to assign the call with
        // this token is a long string - don't insert new lines - different from prod token
        //$userToken = 'AgAAAA**AQAAAA**aAAAAA**UC1ZVw**nY+sHZ2PrBmdj6wVnY+sEZ2PrA2dj6wFk4GgDpCGpQqdj6x9nY+seQ**PM8DAA**AAMAAA**';          
    }
    
    
?>