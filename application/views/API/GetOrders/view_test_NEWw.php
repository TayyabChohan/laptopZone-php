...
if ($entries == 0) {
    ...
    //select * from LZ_SALESLOAD_MT where loading_date = to_timestamp('2/26/2016 5:36:29 AM', 'dd/mm/yyyy hh24:mi:ss')
               ...
               // date_default_timezone_set("Asia/Karachi");
                ...
                $this->db->query("insert into EBAY_NOTEIFICATION (test_pk,E_data,insert_date) values (get_single_primary_key('EBAY_NOTEIFICATION','test_pk'),'inside order array view_test_NEWw ',sysdate)");
    ...
    // ========================== Starts Insertion in MT table ===============================
                 if($row->num_rows() == 0)
                {
                     $this->db->query("insert into EBAY_NOTEIFICATION (test_pk,E_data,insert_date) values (get_single_primary_key('EBAY_NOTEIFICATION','test_pk'),'genrate pk',sysdate)");
    ...
    // INSERT INTO lz_salesload_mt VALUES(147,88,to_date('09/03/2016', 'mm/dd/yyyy'),'09/03/2016',NULL,'09/03/2016','Post in ERP',1)
                    ...
                    $row = $this->db->query($q)->result_array();
            $LZ_SALELOAD_ID = $row[0]['LZ_SALELOAD_ID'];
            $this->db->query("insert into EBAY_NOTEIFICATION (test_pk,E_data,insert_date) values (get_single_primary_key('EBAY_NOTEIFICATION','test_pk'),'pick pk :  $LZ_SALELOAD_ID',sysdate)");
                }
                //var_dump($LZ_SALELOAD_ID);exit;
               // exit;
    // ========================== End Insertion in MT table ===============================
                $tottal_order = count($orders);
                $h = 1;
        foreach ($orders as $order) {
$this->db->query("insert into EBAY_NOTEIFICATION (test_pk,E_data,insert_date) values (get_single_primary_key('EBAY_NOTEIFICATION','test_pk'),'insisde order foreach : $h of $tottal_order ',sysdate)");
$h++;
...
/* ==================== Start Checking Status of Order ============================  */
            ...
            {
                $this->db->query("insert into EBAY_NOTEIFICATION (test_pk,E_data,insert_date) values (get_single_primary_key('EBAY_NOTEIFICATION','test_pk'),'insisde orderstatus check',sysdate)");
    ...
    // ======================= End Ship To Address =========================        
                    ...
                    if ($transactions) {
                        $this->db->query("insert into EBAY_NOTEIFICATION (test_pk,E_data,insert_date) values (get_single_primary_key('EBAY_NOTEIFICATION','test_pk'),'insisde transactions',sysdate)");
                        //echo "Transaction Array \n";
                        // iterate through each transaction for the order
                        foreach ($transactions->Transaction as $transaction) {
                            $this->db->query("insert into EBAY_NOTEIFICATION (test_pk,E_data,insert_date) values (get_single_primary_key('EBAY_NOTEIFICATION','test_pk'),'insisde transactions foreach',sysdate)");
                            ...
                            $this->db->query("insert into EBAY_NOTEIFICATION (test_pk,E_data,insert_date) values (get_single_primary_key('EBAY_NOTEIFICATION','test_pk'),'sold date: $datesold',sysdate)");
                            //  echo "T removed:".$test;
                    ...
                    $this->db->query("insert into EBAY_NOTEIFICATION (test_pk,E_data,insert_date) values (get_single_primary_key('EBAY_NOTEIFICATION','test_pk'),'after getsingle primary key',sysdate)");
    ...
    // ============================ Start Insertion Process =============================
                ...
                // $SHIP_TO_ZIP= '';
                // $SHIP_TO_COUNTRY= '';
                $this->db->query("insert into EBAY_NOTEIFICATION (test_pk,E_data,insert_date) values (get_single_primary_key('EBAY_NOTEIFICATION','test_pk'),'before mannual check : $MANNUAL_YN',sysdate)");
                if(!empty(@$MANNUAL_YN)){
                    $MANNUAL_YN= 1000;
                }else{
                    $MANNUAL_YN= '';
    ...
    // ===================== Start Checking if record already exist ==============================
                ...
                $select_query = "select * from LZ_SALESLOAD_det d where d.order_id = '$ORDER_ID'";
    ...
    // ===================== End Checking if record already exist ===============================
               ...
               if(count($row) == 0){
                    $this->db->query("insert into EBAY_NOTEIFICATION (test_pk,E_data,insert_date) values (get_single_primary_key('EBAY_NOTEIFICATION','test_pk'),'from insert qry:  $ORDER_ID ',sysdate)");
                    ...
                    $this->db->query("insert into EBAY_NOTEIFICATION (test_pk,E_data,insert_date) values (get_single_primary_key('EBAY_NOTEIFICATION','test_pk'),'from update:  $ORDER_ID ',sysdate)");
//,SHIPPING_CHARGES = $SHIPPING_CHARGES
                    ...
                    $qry = "UPDATE LZ_SALESLOAD_DET SET TRACKING_NUMBER ='$TRACKING_NUMBER',SHIPPED_ON_DATE = $SHIPPED_ON_DATE,ORDERSTATUS='$ORDERSTATUS' WHERE ORDER_ID = '$ORDER_ID'";
                }
...
$this->db->query("insert into EBAY_NOTEIFICATION (test_pk,E_data,insert_date) values (get_single_primary_key('EBAY_NOTEIFICATION','test_pk'),'from awaiting shipment:  $OrderLineItemID ',sysdate)");
