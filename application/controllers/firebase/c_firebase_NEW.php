<?php
class c_firebase_NEW extends CI_Controller {
...
public function getFbData()
  {
    $fb_data = $_GET['parm_order'];
    $barcodeArray = $_GET['parm_barcode'];
    $imgArray = $_GET['parm_images'];
    $this->load->library('image_lib');
    var_dump($barcodeArray)."<br>";
    // exit;
    $fb_data = json_decode($fb_data);
    $barcodeArray = json_decode($barcodeArray, true);
    $imgArray = json_decode($imgArray, true);

        $upc = @$fb_data->upc;
        $ean = @$fb_data->mpn;
        $enteredBy = @$fb_data->pic_taker_id;
        $enteredDate = @$fb_data->pic_DateTime;
        // $object = @$fb_data->object;
        $remarks = @$fb_data->remarks;
        $bin = @$fb_data->bin_name;
        // $condition = @$fb_data->condition;
        $condition = @$fb_data->cond_id;
        $conditionRemarks = @$fb_data->cond_remarks; 
        $folderName = @$fb_data->folderBarcode;
        // var_dump($upc , $ean , $enteredBy , $enteredDate   , $bin , $condition , $folderName);
        // exit;
        ...
        echo("Barcodes Count: ".count($barcodeArray)."<br>");
    // exit;
        for($i=0; $i<count($barcodeArray);$i++)
        {
            ...
            if($posted_barcode){
              ...
              // exit;
                    $this->db->query("INSERT INTO LZ_SPECIAL_LOTS (SPECIAL_LOT_ID, BARCODE_PRV_NO, BIN_ID, PIC_DATE_TIME, PIC_BY, LOT_REMARKS, CONDITION_ID, CARD_UPC, INSERTED_AT, INSERTED_BY, FOLDER_NAME,CONDITION_REMRAKS,EAN) VALUES( get_single_primary_key('LZ_SPECIAL_LOTS','SPECIAL_LOT_ID'),'$barcode','$bin_id',$enteredDate,'$enteredBy','$remarks','$condition','$upc',SYSDATE,'$enteredBy','$folderName','$conditionRemarks','$ean')");
                  }else{
                    echo "Already inserted" . "<br>";
                    $this->db->query("UPDATE LZ_SPECIAL_LOTS SET BIN_ID = '$bin_id', PIC_DATE_TIME = $enteredDate , PIC_BY = '$enteredBy', LOT_REMARKS = '$remarks' ,  CONDITION_ID = '$condition', CONDITION_REMRAKS = '$conditionRemarks',  CARD_UPC = '$upc' , FOLDER_NAME = '$folderName' , EAN = '$ean' WHERE BARCODE_PRV_NO = '$barcode'");
                  }
    ...
    if(!is_dir($folder_dir)){
      ...
      foreach (@$imgUrls as $key => $url) {
        ...
        $config['source_image']  = $folder_dir.'/'.$azRange[$i]."_fb_".$img_name.'.jpg';
        $config['new_image']  = $folder_dir."/thumb/".$azRange[$i]."_fb_".$img_name.'.jpg';
        ...
        // copy(@$imageUrl, $thumb_dir.'/'.$azRange[$i]."_fb_".$img_name.'.jpg');
    ...
    //   exit;
      foreach (@$imgUrls as $key => $url) {
        ...
        $config['image_library'] = 'GD2';
        $config['source_image']  = $dir.'/'.$azRange[$i]."_fb_".$img_name.'.jpg';
        $config['new_image']  = $dir."/thumb/".$azRange[$i]."_fb_".$img_name.'.jpg';
        $config['maintain_ratio']    = true;
        $config['width']     = 100;
        $config['height']   = 100;
        $in =$this->image_lib->initialize($config); 
        $result = $this->image_lib->resize($in);
        $this->image_lib->clear();
        // copy(@$imageUrl, $dir."/thumb/".$azRange[$i]."_fb_".$img_name.'.jpg');
