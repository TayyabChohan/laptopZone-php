...
class C_purchasing extends CI_Controller
{
  ...
  public function index()
    {
        $result['data'] = $this->m_purchasing->categorySummary();
    ...
    public function kitComponents()
    {
        ...
        $result['data']               = $this->m_purchasing->kitComponents();
        //$result['getAllObjects']      = $this->m_purchasing->getAllObjects();
        $result['getCategories']      = $this->m_purchasing->getCategories();  
        // echo "<pre>";
        // print_r($result['data']);
        // exit;
        $this->load->view('catalogueToCash/v_kit_components_view', $result);
    }
    public function get_objects()
    {
        $data                         = $this->m_purchasing->get_objects(); 
        echo json_encode($data);
        return json_encode($data);
    }
    public function get_brands()
    {
        $data                         = $this->m_purchasing->get_brands(); 
        echo json_encode($data);
        return json_encode($data);
    } 
    public function get_mpns()
    {
        $data                         = $this->m_purchasing->get_mpns(); 
        echo json_encode($data);
        return json_encode($data);
    }
    public function get_upc()
    {
        $data                         = $this->m_purchasing->get_upc(); 
        echo json_encode($data);
        return json_encode($data);
    }
    public function savekitComponents()
    {
         $data = $this->m_purchasing->savekitComponents();
            echo json_encode($data);
            return json_encode($data); 
    }
    public function addKitComponent()
    {
         $data = $this->m_purchasing->addKitComponent();
            echo json_encode($data);
            return json_encode($data); 
    }
    ...
    public function cp_delete_component()
    {
        ...
        if($data==1){
              ...
              $this->session->set_flashdata('error',"Component deletion failed!");
                ...
                $this->session->set_flashdata('warning', "Warning: This is main object first delete other/s same objects.");
            }
    ...
    public function up_delete_component()
    {
        ...
        $mpn_id = $this->uri->segment(5);
        $cata_id = $this->uri->segment(6);
        $data = $this->m_purchasing->up_delete_component();
        if($data==1){
              $this->session->set_flashdata('success', "Component deleted Successfully!"); 
            }elseif($data==0) {
              $this->session->set_flashdata('error',"Component deletion failed!");
            ...
            redirect(base_url()."catalogueToCash/c_purchasing/updateEstimate/".$cat_id."/".$mpn_id."/".$cata_id."/1111");  
    }
...
public function update_components_session(){
        $this->m_purchasing->update_components_session();  
    }
/*=================================================
...
public function getSellerDescription(){
    $data = $this->m_purchasing->getSellerDescription();
    echo json_encode($data);
    return json_encode($data); 
}
public function createObject(){
      $data = $this->m_purchasing->createObject();
      echo json_encode($data);
      return json_encode($data);
    }
public function addCataMpn(){
      $data = $this->m_purchasing->addCataMpn();
      echo json_encode($data);
      return json_encode($data);
    }
public function estimate_detail(){
    $data = $this->m_purchasing->estimate_detail();
    echo json_encode($data);
    return json_encode($data);
}
 public function addRssUrls(){
      $data = $this->m_purchasing->addRssUrls();
      echo json_encode($data);
      return json_encode($data);

    }
public function getRssUrlData(){
      $data = $this->m_purchasing->getRssUrlData();
      echo json_encode($data);
      return json_encode($data);

    }
public function updateRssUrl(){
      $data = $this->m_purchasing->updateRssUrl();
      echo json_encode($data);
      return json_encode($data);

    }
    public function get_status_remarks(){
      $data = $this->m_purchasing->get_status_remarks();
      echo json_encode($data);
      return json_encode($data);
